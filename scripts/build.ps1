...
# Display build info
$version = Get-ContensiveVersion -DeploymentRoot 'C:\Deployments\aoCodeMirrorEditor'
Write-Host ""
Write-Host "========================================"
Write-Host "  CodeMirror Editors Build"
...
Invoke-ContensiveBuild `
    -CollectionName    'CodeMirror Editors' `
    -CollectionPath    "$projectRoot\collections\CodeMirror Editors" `
    -SolutionPath      "$projectRoot\source\CodeMirrorEditor.sln" `
    -BinPath           "$projectRoot\source\CodeMirrorEditor\bin\$Configuration\netstandard2.0" `
    -Configuration     $Configuration `
    -DeploymentRoot    'C:\Deployments\aoCodeMirrorEditor' `
    -CleanFolders      @(
                           "$projectRoot\source\CodeMirrorEditor\bin"
                           "$projectRoot\source\CodeMirrorEditor\obj"
                       ) `
